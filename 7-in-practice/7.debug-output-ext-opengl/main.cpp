...
int main()
{
    ...
    if (flags & GL_CONTEXT_FLAG_DEBUG_BIT)
    {
        ...
        glDebugMessageInsert(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_MEDIUM, -1, "this is a test error message");
    }
    ...
    // -------------------------
    Shader shader("screen_shader.vs", "screen_shader.fs");
    Shader simpleDepthShader("depth_shader.vs", "depth_shader.fs", "depth_shader.gs");    
    ...
    for (unsigned int i = 0; i < 6; ++i)
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, SHADOW_WIDTH, 
                    SHADOW_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL); // assign each cube face as depth texture
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // float radius = 3.0;
        // lightPos.z = static_cast<float>(sin(glfwGetTime() * 0.5) * radius);
        ...
        simpleDepthShader.setMat4("camera_view", view);
        simpleDepthShader.setMat4("camera_projection", projection);
        ...
        shader.setVec3("lightColor", glm::vec3(0.3, 0.3, 0.3));
