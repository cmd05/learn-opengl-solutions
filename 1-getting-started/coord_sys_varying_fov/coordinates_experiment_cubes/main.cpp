...
float ANGLE_FOV_Y = 90;
float PROJ_MAT_ASPECT_RATIO = (float)SCREEN_CURR_WIDTH / SCREEN_CURR_HEIGHT;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /// Projection Matrix
        {
            ...
            float fov_y = glm::radians(ANGLE_FOV_Y), n = 0.1f, f = 100.0f;
            float tan_half_angle = tanf(fov_y / 2);

            projection = glm::transpose(glm::mat4{
                1.0f / (tan_half_angle * PROJ_MAT_ASPECT_RATIO), 0.0f, 0.0f, 0.0f,
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP)) {
        ...
        if (ANGLE_FOV_Y >= 175.0f)
            ANGLE_FOV_Y = 175.0f;

        std::cout << ANGLE_FOV_Y << '\n';
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN)) {
        ANGLE_FOV_Y -= 0.01f;

        if (ANGLE_FOV_Y <= 10.0f)
            ANGLE_FOV_Y = 10.0f;
        
        std::cout << ANGLE_FOV_Y << '\n';
    }
    if (glfwGetKey(window, GLFW_KEY_RIGHT)) {
        PROJ_MAT_ASPECT_RATIO += 0.001f;

        if (PROJ_MAT_ASPECT_RATIO >= 5.0f)
            PROJ_MAT_ASPECT_RATIO = 5.0f;

        std::cout << PROJ_MAT_ASPECT_RATIO << '\n';
    }
    if (glfwGetKey(window, GLFW_KEY_LEFT)) {
        PROJ_MAT_ASPECT_RATIO -= 0.001f;

        if (PROJ_MAT_ASPECT_RATIO <= 0.1f)
            PROJ_MAT_ASPECT_RATIO = 0.1f;
