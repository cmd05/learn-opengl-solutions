...
// ------------------------------------------------------------------
    ...
    // create a *multisampled color attachment texture* to `framebuffer`
    ...
    // create a (also multisampled) *renderbuffer object* for *depth and stencil attachments* to `framebuffer`
    unsigned int rbo;
    ...
    // configure second post-processing (intermediate) framebuffer
    unsigned int intermediateFBO;
    glGenFramebuffers(1, &intermediateFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, intermediateFBO);

    // create a color attachment texture (not multisampled)
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, screenTexture, 0); // bound to `intermediateFBO`
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        glClearColor(0.1f, 0.1f, 0.1f, 1.0f); // clears the color buffer
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
